...
**Privacy and security:** Screen Ants runs locally on your Windows PC. It does not use the
network, upload data, collect telemetry, or persist screenshots. To create the visual
effect, it temporarily captures the screen in memory, detects visual elements locally, and
draws a transparent click-through overlay. It may also move the mouse cursor as part of
the "ants carrying the cursor" animation. Settings are stored only in `config.json` next
to the executable, and crash details may be written to `error.log` next to the executable.
...
- **The Queen** - a giant queen comes out to eat food the workers deliver, and on a timer she charges out herself to forage a colourful chunk of your screen, hauling it home with a squad of workers at 1.5x speed.
